...
void Table::insert(std::vector<char*> & newRecord) {
    if (newRecord.size() != tableInfo.fieldNumber) {
...
void Table::writeHeader() {
    ...
    if (pFieldStruct) {
        if (write(fd, pFieldStruct, sizeof(Field) * tableInfo.fieldNumber) != (sizeof(Field) * tableInfo.fieldNumber)) {
            throw std::logic_error("Can not write to the file.");
        }
    } else {
        throw std::logic_error("Can not write to the file.");    
...
void Table::readHeader() {
    ...
    if (read(fd, pFieldStruct, sizeof(Field) * tableInfo.fieldNumber) != (sizeof(Field) * tableInfo.fieldNumber)) {
        throw std::logic_error("Can not write to the file.");
    }
}
...
void Table::getLong(const string & fieldName, long & value) {
    ...
    if (lseek(fd, currentPos + sizeof(Links), SEEK_SET) != (currentPos + sizeof(Links))) {
        throw std::logic_error("Can not move to the position in the file.");
    }
...
void Table::putText(const string & fieldName, const string & pvalue) {
    ...
    for (long i = 0; i < tableInfo.fieldNumber; i++) {
        ...
        if (!strcmp(fieldName.c_str(), pFieldStruct[i].name)) {
            ...
            if (pvalue.size() > pFieldStruct[i].len) {
...
void Table::deleteRec() {
    ...
    if(links.prevOffset == -1) {
        ...
        if (lseek(fd, links.prevOffset + sizeof(links.prevOffset), SEEK_SET) != (links.prevOffset + sizeof(links.prevOffset))) {
            throw std::logic_error("Can not move to the position in the file.");
        }
